...
PulserRangeStep = 5     # <= Step Size of Pulser Sweep Range
NofIterationsTOT = 20   # <= Number of Iterations for each Pulser Value
DelayValueTOT = 2300       # <= Value of Programmable Delay for TOT Pulser Sweep
...
if nTOA_TOT_Processing == 1:
    ...
    for i in range(PulserRangeL, PulserRangeH):
        ...
        exec("%s = %r" % ('HitDataTOA%d' %i, HitDataTOA))

        if len(HitDataTOA) > 0:
            DataMeanTOA.append(np.mean(HitDataTOA, dtype=np.float64))
        else:
            DataMeanTOA.append(0)
